...
function elasticRods(bendModulus=1, twistModulus=1, totalTwist=pi)
    ...
    loop = Observable(Point3f.(eachcol([verts verts[:, 1]])))
    bases = Observable(Point3f.(eachcol(curveData.midpointsR)))
    vecs = Observable(Point3f.(eachcol(curveData.u[:, 1:end-1])))
    ...
    for i = 1:10000
        ...
        if mod(i - 1, 40) == 0
            sleep(0.001)
            loop[] = Point3f.(eachcol([verts verts[:, 1]]))
            bases[] = Point3f.(eachcol(curveData.midpointsR))
            vecs[] = Point3f.(eachcol(curveData.u[:, 1:end-1]))
        end
    end
end
